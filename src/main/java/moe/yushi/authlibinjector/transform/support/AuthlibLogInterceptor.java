...
public class AuthlibLogInterceptor implements TransformUnit {
    private static Set<ClassLoader> interceptedClassloaders
    ...
    public static void onClassLoading(ClassLoader classLoader) {
        ...
        try {
            classLogManager
                = classLoader.loadClass("org.apache.logging.log4j.LogManager");
    ...
    private static void registerLogHandle(ClassLoader cl)
        ...
        Class<?> classAppenderRef
            = cl.loadClass("org.apache.logging.log4j.core.config.AppenderRef");
        ...
        Class<?> classLoggerConfig
            = cl.loadClass("org.apache.logging.log4j.core.config.LoggerConfig");
        Class<?> classConfiguration
            = cl.loadClass("org.apache.logging.log4j.core.config.Configuration");
        Class<?> classPatternLayout
            = cl.loadClass("org.apache.logging.log4j.core.layout.PatternLayout");
        Class<?> classConsoleAppender
            = cl.loadClass("org.apache.logging.log4j.core.appender.ConsoleAppender");

        Object loggerContext = cl.loadClass("org.apache.logging.log4j.LogManager")
                                   .getDeclaredMethod("getContext", boolean.class)
                                   .invoke(null, false);
        Object configuration = cl.loadClass("org.apache.logging.log4j.core.LoggerContext")
                                   .getMethod("getConfiguration")
                                   .invoke(loggerContext);

        Object patternLayout;
        try {
            Object builder
                = classPatternLayout.getDeclaredMethod("newBuilder").invoke(null);
            Class<?> classBuilder = cl.loadClass(
            ...
            // prior to
            // https://github.com/apache/logging-log4j2/commit/7aabb11111c69f452d674b00845b66b82c80afa4
            Map<String, Object> values = new HashMap<>();
            values.put("alwaysWriteExceptions", true);
            values.put("noConsoleNoAnsi", true);
            patternLayout = invokeCreateMethod(
                classPatternLayout, "createLayout", configuration, values
            );
        }
        ...
        try {
            Object buider
                = classConsoleAppender.getDeclaredMethod("newBuilder").invoke(null);
            Class<?> classBuilder = cl.loadClass(
                "org.apache.logging.log4j.core.appender.ConsoleAppender$Builder"
            );
            classBuilder.getMethod("withLayout", classLayout)
                .invoke(buider, patternLayout);
            ...
            appender = invokeCreateMethod(
                classConsoleAppender, "createAppender", configuration, values
            );
        }
        ...
        Object appenderRef;
        {
            ...
            appenderRef = invokeCreateMethod(
                classAppenderRef, "createAppenderRef", configuration, values
            );
        }
        ...
        try {
            Object builder
                = classLoggerConfig.getDeclaredMethod("newBuilder").invoke(null);
            Class<?> classBuilder
                = cl.loadClass("org.apache.logging.log4j.core.config.LoggerConfig$Builder"
                );
            classBuilder.getMethod("withConfig", classConfiguration)
                .invoke(builder, configuration);
            classBuilder.getMethod("withAdditivity", boolean.class)
                .invoke(builder, false);
            classBuilder.getMethod("withLevel", classLevel)
                .invoke(builder, classLevel.getDeclaredField("ALL").get(null));
            classBuilder.getMethod("withLoggerName", String.class)
                .invoke(builder, loggerName);
            classBuilder.getMethod("withIncludeLocation", String.class)
                .invoke(builder, authlibPackageName);
            classBuilder.getMethod("withRefs", appenderRefs.getClass())
                .invoke(builder, appenderRefs);
            ...
            loggerConfig = invokeCreateMethod(
                classLoggerConfig, "createLogger", configuration, values
            );
        }

        classLoggerConfig.getMethod("addAppender", classAppender, classLevel, classFilter)
            .invoke(loggerConfig, appender, null, null);
        try {
            classConfiguration.getMethod("addAppender", classAppender)
                .invoke(configuration, appender);
        } catch (NoSuchMethodException e) {
            ((Map) classConfiguration.getMethod("getAppenders").invoke(configuration))
                .put(appenderName, appender);
        }
        try {
            classConfiguration.getMethod("addLogger", String.class, classLoggerConfig)
                .invoke(configuration, authlibPackageName, loggerConfig);
        } catch (NoSuchMethodException e) {
            // prior to
            // https://github.com/apache/logging-log4j2/commit/f9744033b93e2fe1f52a27c66f24f53cf44939a2
            // bypass the check in
            // https://github.com/apache/logging-log4j2/blob/log4j-2.0-beta9/log4j-core/src/main/java/org/apache/logging/log4j/core/config/BaseConfiguration.java#L554
            Class<?> classBaseConfiguration
                = cl.loadClass("org.apache.logging.log4j.core.config.BaseConfiguration");
            ...
            Method methodSetParents
                = classBaseConfiguration.getDeclaredMethod("setParents");
            methodSetParents.setAccessible(true);
            methodSetParents.invoke(configuration);
        }
        cl.loadClass("org.apache.logging.log4j.core.LoggerContext")
            .getMethod("updateLoggers", classConfiguration)
            .invoke(loggerContext, configuration);
    }

    private static Object invokeCreateMethod(
        Class<?> owner, String methodName, Object config, Map<String, Object> values
    ) throws ReflectiveOperationException {
        ...
        Method method
            = Stream.of(owner.getDeclaredMethods())
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            Annotation annotation
                = parameters[i].getDeclaredAnnotation(classPluginAttribute);
            if (annotation != null) {
                    ...
                    if ("<clinit>".equals(name)) {
                        return new MethodVisitor(
                            ...
                            public void visitCode() {
                                super.visitCode();
                                super.visitLdcInsn(
                                    Type.getType("L" + className.replace('.', '/') + ";")
                                );
                                ...
                                ctx.invokeCallback(
                                    mv, AuthlibLogInterceptor.class, "onClassLoading"
                                );
                                ctx.markModified();
                            }
                        };
