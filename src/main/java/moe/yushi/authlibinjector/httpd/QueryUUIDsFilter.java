...
public class QueryUUIDsFilter implements URLFilter {
    ...
    public boolean canHandle(String domain) {
        return domain.equals("api.mojang.com")
            || domain.equals("api.minecraftservices.com");
    }

    @Override
    public Optional<Response> handle(String domain, String path, IHTTPSession session)
        throws IOException {
        if ((domain.equals("api.mojang.com") && path.equals("/profiles/minecraft")
             && session.getMethod().equals("POST"))
            || (domain.equals("api.minecraftservices.com")
                && path.equals("/minecraft/profile/lookup/bulk/byname")
                && session.getMethod().equals("POST"))) {
    ...
    private Map<String, UUID> performQuery(Set<String> names) {
        ...
        if (!mojangNames.isEmpty()) {
            mojangClient.queryUUIDs(mojangNames).forEach((name, uuid) -> {
    ...
    static UUID maskUUID(UUID uuid) {
        ...
        return new UUID(
            uuid.getMostSignificantBits() | MSB_MASK, uuid.getLeastSignificantBits()
        );
    }
