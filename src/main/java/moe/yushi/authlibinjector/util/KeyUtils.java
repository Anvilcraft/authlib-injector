...
public final class KeyUtils {
    public static byte[] decodePEMPublicKey(String pem) throws IllegalArgumentException {
        ...
        if (pem.startsWith(header) && pem.endsWith(end)) {
            return Base64.getDecoder().decode(
                pem.substring(header.length(), pem.length() - end.length())
            );
        } else {
    ...
    public static PublicKey parseX509PublicKey(byte[] encodedKey)
    ...
    public static PublicKey parseSignaturePublicKey(String pem)
