...
public final class Logging {
    ...
    private static final Pattern CONTROL_CHARACTERS_FILTER
    ...
    private static FileChannel openLogFile() {
        ...
        try {
            ...
            channel.truncate(0);
            String logHeader
                = "Logging started at " + Instant.now() + System.lineSeparator();
    ...
    public static void log(Level level, String message, Throwable e) {
        ...
        if (logfile != null) {
            try {
                logfile.write(
                    Charset.defaultCharset().encode(log + System.lineSeparator())
                );
            } catch (IOException ex) {
                out.println(
                    "[authlib-injector] [ERROR] Error writing to log file: " + ex
                );
            }
        }
    }
