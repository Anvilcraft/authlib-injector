...
public final class Config {
    ...
    private static void initDebugOptions() {
        ...
        if ("all".equals(prop)) {
            prop = "";
            log(WARNING,
                "'-Dauthlibinjector.debug=all' is deprecated, use "
                + "'-Dauthlibinjector.debug' instead");
        }
        if (prop == null) {
    ...
    private static void initMojangProxy() {
        ...
        if (prop == null) {
            ...
            if (prop == null) {
                ...
                log(WARNING,
                    "'-Dauthlibinjector.mojang.proxy=' is deprecated, use "
                    + "'-Dauthlibinjector.mojangProxy=' instead");
            }
        }

        Matcher matcher
            = Pattern.compile("^(?<protocol>[^:]+)://(?<host>[^/]+)+:(?<port>\\d+)$")
                  .matcher(prop);
        if (!matcher.find()) {
